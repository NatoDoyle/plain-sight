...
**As the defense agent (Claude Code):** the repo ships the agent as a skill — `.claude/skills/defense-agent/SKILL.md` operationalizes the spec. Clone the repo, open [Claude Code](https://claude.com/claude-code) in the folder, and describe the situation (or invoke `defense-agent` directly); the session becomes the defense agent, with safety escalation and caveats built in.

## Status — ✅ complete

All **166 topics across 9 tiers** are written, cross-linked, evidence-graded, and safety-checked. `python3 tools/kb.py validate` → **166 files, 0 errors, 0 warnings**. Snapshot: **37** mechanisms · **68** tactics · **10** dynamics · **12** contexts · **6** profiles · **9** vulnerabilities · **15** defenses · **9** meta/synthesis. **1,429** typed connections; **57** files carry crisis-safety guidance with verified hotline resources; **84** "innocent look-alike" contrast concepts guard against false positives; evidence honestly graded (40 established / 92 supported / 14 clinical / 7 contested / 6 folk), with every contested and folk construct flagged inline.

**Tier 9 — Cognitive Bias Codex integration (2026-08-18).** The complete Benson/Manoogian codex — **189 listed entries, 188 unique** — is mapped in `taxonomy/bias-codex-index.md`, every row resolving to a KB home with an evidence grade and an exploitation-relevance label (*lever · amplifier · observer-side · not-a-lever*). 14 new clustered mechanism files, 6 existing files extended, 13 pointer aliases. The grading is the point: **95 established, 54 supported, 22 contested, 12 folk, 5 informal-logic entries, and one that is not a bias at all** — roughly a fifth of the famous wheel is contested, folk, or miscategorized, and this KB says so per entry rather than averaging it away. Normalcy bias is graded `folk` and its famous 80% statistic appears once, as a traced example of laundering; the Dunning-Kruger "Mount Stupid" curve is disowned as internet folklore absent from the 1999 paper; Occam's razor is given no grade because it is a sound principle the codex miscategorized. See `ROADMAP.md` for the tier-by-tier map and `taxonomy/coverage-audit.md` for an honest gap analysis.
...
**🔧 Maintenance / QA pass (2026-07-01): 151/151.** A gap-and-consistency audit fixed inventory drift across README/ROADMAP/`coverage-audit`, hardened `kb.py` (safety-flag symmetry + phantom-link warnings), downgraded 236 contrast-concept wikilinks to remove Obsidian phantom nodes, added ~18 missing graph edges, backfilled 42 per-topic bibliographies, and authored 3 new topics (`tone-policing`, `spiritual-abuse`, `overton-window-shifting`) plus the `emergent-insights` graph analysis. Now **1,318 edges · 53 safety-flagged files · 79 contrast concepts**.
