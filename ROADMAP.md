...
**Status:** ✅ **ALL TIERS COMPLETE — 166/166.** Tiers 0–8 (151 topics) plus **Tier 9 — Cognitive Bias Codex integration, completed 2026-08-18**: 14 new clustered mechanism files, 6 existing files extended, 13 pointer aliases, and [[bias-codex-index]] mapping all **189 listed codex entries (188 unique)** to a home that exists, each with an evidence grade and an exploitation-relevance label. Snapshot: 3 foundations · 37 mechanisms · 68 tactics · 10 dynamics · 12 contexts · 6 profiles · 9 vulnerabilities · 15 defenses · 9 meta/synthesis. Validates with 0 errors and 0 warnings; 1,429 compiled edges; 57 safety-flagged files; 84 false-positive contrast concepts. Codex evidence distribution — 95 established / 54 supported / 22 contested / 12 folk / 5 informal-logic / 1 not-a-bias — roughly a fifth of the codex is contested, folk, or miscategorized, and the KB says so per entry. Remaining work is maintenance: localize resources beyond US/UK, refresh fast-moving files, clear the per-file pre-`complete` citation spot-checks logged in `sources/bibliographies/` (see [[coverage-audit]]). · last updated 2026-08-18
...
- [x] `bias-codex-index` (taxonomy/meta) — pinned 188-entry enumeration; entry → KB-home map with evidence + exploitation-relevance columns; the completeness gate
...
- [x] `time-distortions` — hyperbolic discounting (present bias), planning fallacy, impact/projection/pessimism bias, declinism, appeal to novelty, pro-innovation bias, time-saving bias, well-traveled-road effect
