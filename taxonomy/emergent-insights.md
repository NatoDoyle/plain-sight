...
Computed 2026-08-18 from `graph/edges.yaml` (1,429 directed edges across 11 of the 12 typed relations — `variant-of` is defined but currently unused) and the frontmatter of all 166 content files, using the same parse `tools/kb.py` uses. Node "degree" = the count of typed edges touching a node.
...
Of 243 `exploits` edges, the two most-pulled psychological levers are [[trust-mechanics]] (21) and [[attachment-needs]] (19) — together ~16.5% of all exploitation, well ahead of [[fear-exploitation]] (14), [[social-proof]] (13), and [[guilt-leverage]] (13). The distribution is long-tailed: a few levers do most of the work.
...
68 tactics exploit 37 mechanisms in enormous variety, but the *counter* side is radically centralized. Two defenses absorb ~59% of all 242 `countered-by` edges: [[detection-heuristics]] (83) and [[verification-rituals]] (59) — a share **re-verified 2026-08-18**, unmoved by Tier 9. Add [[boundary-scripts]] (30) and [[documentation-practices]] (25) and four defenses cover ~81%.
...
The `escalates-to` layer has only seven edges, and **all seven lead — directly or in two hops — to [[coercive-control]]** (**re-verified 2026-08-18: still 7/7, still 100%**): [[gaslighting]], [[silent-treatment]], [[veiled-threats]], [[emotional-blackmail]], [[escalation-entrapment]], and [[spiritual-abuse]] point straight at it, and [[guilt-tripping]] routes there via emotional-blackmail. The `enables` layer adds [[isolation-tactics]] → coercive-control.
...
`distinguished-from` is the single largest relation: 436 edges — **~30% of the whole graph, a share re-verified 2026-08-18 and unchanged through Tier 9** — backed by 84 registered "innocent look-alike" contrast concepts. The most-guarded look-alikes are *honest disagreement* (11), *genuine rapport* (10), *honest hurt* (10), and *hard bargaining* (8).
...
Cross-tabbing degree against the `evidence` grade exposes a clean inversion. The catchy, viral, "therapy-speak" tactic names are disproportionately the weakest-evidenced: [[moving-goalposts]] (folk, degree 27), [[future-faking]] (folk, 27), [[therapy-speak-weaponization]] (folk, 18), [[boundary-testing]] (folk, 17), [[negging]] (contested, 16), [[radicalization-pipeline]] (contested, 16), [[abuse-cycle]] (contested, 14), [[weaponized-incompetence]] (folk, 13), [[machiavellianism]] (contested, 10).

Meanwhile the mechanism layer runs the other way: **34 of the 37 mechanisms — the least viral, most "boring" concepts (reciprocity, anchoring, memory fallibility) — are graded `established`.** So the "why it works" foundation is close to empirically bulletproof, while the memorable "what they do" naming layer is exactly where the science is thinnest.

The three exceptions are worth naming rather than rounding away, because the same force made them. [[choice-simplification]], [[risk-misperception]] and [[time-distortions]] all arrived with Tier 9, assembled out of Cognitive Bias Codex entries, and all three carry **`supported`** rather than `established` precisely *because the codex's own entries turned out to be weak once read*: normalcy bias dissolving into a single 1994 conceptual paper plus a blog-invented "80%," bike-shedding sourced to a published joke, pessimism bias with no distinct construct behind it, the dominance-reversal demonstration of less-is-better failing to replicate. These are famous names whose underlying literature would not carry the top grade. That is the fame-versus-evidence gradient reappearing *inside* the mechanism layer rather than contradicting it — the constructs the KB inherited on reputation are exactly the ones whose grade fell when the sources were pulled. The pattern is not "mechanisms are established and tactics are not." It is that **grades track how hard the source literature was actually looked at, and fame is a poor proxy for that.**

The KB inverts the public's confidence gradient: the terms people deploy most confidently are the ones it most flags as "useful vocabulary, thin research." **Emergent** (a cross-tab of two frontmatter fields the KB never joins).

## 6. Severity concentrates in the home, not the exotic con

All five `critical`-severity files are intimate or predatory: [[coercive-control]], [[abuse-cycle]], [[isolation-tactics]], [[grooming-sequence]], and [[self-harm-threats]]. The busiest arenas by tactic count are intimate-relationships (48), family-parents (40), and workplace-bosses (34) — dwarfing scams (22) or cults (20). Nearly all 57 safety-flagged files cluster here.
...
The most-connected *tactic* is not gaslighting — it is [[love-bombing]] (total degree 37, **re-verified 2026-08-18**), which `precedes` both [[isolation-tactics]] and [[conditional-love]], exploits four mechanisms, and spans four arenas. The top of the distribution is tight rather than commanding, and honesty requires saying so: [[isolation-tactics]] ties it at 37 and [[darvo]] sits one behind at 36, so this is a near-three-way tie, not a lone hub. Darvo still carries the highest co-occurrence degree of any tactic (9) and `enables` history-rewriting.
...
> **This section predates Tier 9 and has not been re-derived.** It was written against a corpus of 23 mechanisms, every one of them a lever a manipulator pulls. The KB now carries 37, and the 14 cognitive-bias clusters Tier 9 added are the first part of the corpus that catalogues distortions **nobody has to induce**: of the 152 codex rows homed in those clusters, [[bias-codex-index]] labels 92 `lever` but **22 `observer-side` and 21 `not-a-lever`** — 43 entries that are errors in the *observer's own* judgement rather than moves against them ([[pattern-illusions]], [[attribution-errors]], [[memory-retrieval-quirks]], [[stereotyping-essentialism]] carry most of them). Levers still dominate; the point is that a non-lever layer now exists at all. That bears directly on the over-detection bullet below: a catalogue of ways a worried person manufactures a pattern that is not there cuts **both** ways — it gives the agent vocabulary for a target's misreadings, and it is the part of the corpus most easily turned into a way of talking someone out of a real one. How the agent should weight it is an open question, not a settled implication.
